...
create table public.labour_ledger (
 id uuid primary key default gen_random_uuid(), user_id uuid not null references public.profiles(id) on delete cascade,
...
alter table public.client_ledger enable row level security; alter table public.labour_contractors enable row level security; alter table public.labour_ledger enable row level security;
create policy "client_ledger_select" on public.client_ledger for select to authenticated using(user_id=auth.uid() or public.is_admin());
create policy "client_ledger_insert" on public.client_ledger for insert to authenticated with check(user_id=auth.uid() or public.is_admin());
create policy "client_ledger_update" on public.client_ledger for update to authenticated using(user_id=auth.uid() or public.is_admin()) with check(user_id=auth.uid() or public.is_admin());
create policy "client_ledger_delete" on public.client_ledger for delete to authenticated using(user_id=auth.uid() or public.is_admin());
create policy "labour_contractors_select" on public.labour_contractors for select to authenticated using(user_id=auth.uid() or public.is_admin());
create policy "labour_contractors_insert" on public.labour_contractors for insert to authenticated with check(user_id=auth.uid() or public.is_admin());
create policy "labour_contractors_update" on public.labour_contractors for update to authenticated using(user_id=auth.uid() or public.is_admin()) with check(user_id=auth.uid() or public.is_admin());
create policy "labour_contractors_delete" on public.labour_contractors for delete to authenticated using(user_id=auth.uid() or public.is_admin());
create policy "labour_ledger_select" on public.labour_ledger for select to authenticated using(user_id=auth.uid() or public.is_admin());
create policy "labour_ledger_insert" on public.labour_ledger for insert to authenticated with check(user_id=auth.uid() or public.is_admin());
create policy "labour_ledger_update" on public.labour_ledger for update to authenticated using(user_id=auth.uid() or public.is_admin()) with check(user_id=auth.uid() or public.is_admin());
create policy "labour_ledger_delete" on public.labour_ledger for delete to authenticated using(user_id=auth.uid() or public.is_admin());
